...
def calc_min(candles):
  ...
  rsi70 = params.rsi(ind.rename(columns={"RSI9": "RSI"})[["id", "RSI"]].copy(), ass=70)
  # rsi50 = params.rsi(ind.rename(columns={"RSI9": "RSI"})[["id", "RSI"]].copy(), ass=50)
  # rsi30 = params.rsi(ind.rename(columns={"RSI9": "RSI"})[["id", "RSI"]].copy(), ass=30)
...
def calc_hour(candles):
  ...
  rsi70 = params.rsi(ind.rename(columns={"RSI14": "RSI"})[["id", "RSI"]].copy(), type="hour", ass=70)
  # rsi50 = params.rsi(ind.rename(columns={"RSI14": "RSI"})[["id", "RSI"]].copy(), type="hour", ass=50)
  ...
  df_h.drop(range(0, df_h["сEMA_hour"].first_valid_index()), inplace=True)

  return df_h, int(df_h.loc[df_h["сEMA_hour"].first_valid_index(), "id"])


def get_candle_analytics(candles, in_base=False):
  df_m = calc_min(candles)
  df_h, valid_id = calc_hour(candles)
  df_m.drop(range(0, valid_id), inplace=True)

  df = pd.merge(df_m, df_h, on='id', how="outer")
  # print(tabulate(df.loc[0:10], headers='keys', tablefmt='psql'))
